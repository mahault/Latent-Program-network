...
class SpatialEncoder(nn.Module):
    ...
    def forward(self, input_grids: torch.Tensor, output_grids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        input_flat = input_grids.reshape(batch_size * num_examples, H, W).long()
        output_flat = output_grids.reshape(batch_size * num_examples, H, W).long()
        ...
        combined = combined.reshape(batch_size, num_examples, -1)
...
class SpatialDecoder(nn.Module):
    ...
    def forward(self, z: torch.Tensor, input_grid: torch.Tensor) -> torch.Tensor:
        ...
        input_encoded = input_encoded.reshape(batch_size, -1)  # [batch, 128*16]
